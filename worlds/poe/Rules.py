...
#        print("_______________________________________ YOU ARE IN ACT " + str(act) + " _______________________________________")
    ...
    reachable = gear_count >= gear_amount and \
           flask_count >= flask_amount and \
           gem_slot_count >= gem_slot_amount and \
           skill_gem_count >= skill_gem_amount
    if not reachable:
        print (f"[DEBUG] Act {act} not reachable with gear: {gear_count}/{gear_amount}, flask: {flask_count}/{flask_amount}, gem slots: {gem_slot_count}/{gem_slot_amount}, skill gems: {skill_gem_count}/{skill_gem_amount}")
        if _debug:
            print(f"[DEBUG] expecting Act {act} - Gear: {gear_amount}, Flask: {flask_amount}, Gem Slots: {gem_slot_amount}, Skill Gems: {skill_gem_amount}")
            print(f"[DEBUG] we have   Act {act} - Gear: {gear_count}, Flask: {flask_count}, Gem Slots: {gem_slot_count}, Skill Gems: {skill_gem_count}")
